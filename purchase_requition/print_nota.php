...
if ($id_user != ""){  //Kondisi awal untuk hak akses
    ...
    $sql = "SELECT pr.*, r.rolename, u.name
	...
	$pdf->Ln();
	$pdf->Cell(118);
	$pdf->Cell(10,-5,'Jl. Asem Baris Raya No A11 Tebet Jakarta Selatan');
	$pdf->Ln();
	$pdf->Cell(112);
	...
	$pdf->Cell(210,30,'FORM PENGAJUAN BUDGET', 0, 0, 'C');
    $pdf->SetFont('Times','',11);
    $pdf->Ln();
	$pdf->Cell(15);
    $pdf->Cell(20,-10,'Divisi');
	$pdf->Cell(20);
    $pdf->Cell(15,-10,':  '.$result['rolename']);
    ...
    $sql = "SELECT g.name as goods, dpr.quantity, g.quantity_unit,  u.name as unit,
        ...
        CAST((dpr.quantity/g.quantity_unit)*((SELECT
            ...
            WHEN COUNT(id_fix_price) > 0 THEN fixed_price
            ELSE g.price_estimate END
            FROM tb_fix_price WHERE id_goods = g.id_goods ORDER BY created DESC LIMIT 1)) AS INT) as total        FROM tb_detail_pr_item dpr
    ...
    $pdf->Row(array('Sub Total', "Rp " . number_format($subtotal, 0, ',', '.')));
    $pdf->Cell(15);
    $pdf->SetFont('Times','BU',11);
    $pdf->Cell(20, 10,'CARA PEMBAYARAN');
    $pdf->Ln();
    $sql = "SELECT id_account, method FROM tb_payment WHERE id_purchase_requition = :idpr";
    $query=$dbcon->prepare($sql);
    $query->bindParam("idpr", $idpr, PDO::PARAM_STR);
    $query->execute();
    $result = $query->fetch(PDO::FETCH_ASSOC);
    $id_account = $result['id_account'];
    $method = $result['method'] == 1 ? "CASH" : "TRANSFER";
    $pdf->SetFont('Times','B',11);
    $pdf->Cell(15);
    $pdf->Cell(20, 3, $method);
    if($result['method'] == 2){
        $sql = "SELECT account, behalf, account_bank FROM tb_account WHERE id_account=:id_account";
        $query=$dbcon->prepare($sql);
        ...
        $pdf->Cell(15);
        $pdf->Cell(15, 10, 'a/n : ');
        $pdf->Cell(50, 10, $result['behalf']);
        $pdf->Cell(25, 10, 'Nama Bank : ');
        $pdf->Cell(40, 10, $result['account_bank']);
        $pdf->Cell(20, 10, 'Rekening : ');
        $pdf->Cell(50, 10, $result['account']);
    }
    $pdf->Ln();
    $pdf->Cell(15);
    $pdf->SetFont('Times','',10);
    $pdf->Cell(25, 5, 'Mengetahui,');
    $pdf->Ln();
    $pdf->Cell(15);
    $pdf->Cell(40, 5, 'Pemohon/Pembuat,');
    $pdf->Cell(40, 5, 'Supervisor,');
    $pdf->Cell(39, 5, '*Manager,');
    $pdf->Cell(40, 5, '**Direktur,');
    $pdf->Cell(38, 5, 'Keuangan,');
    $pdf->Ln();
    $pdf->Ln();
    $pdf->Ln();
    $pdf->Cell(18);
    ...
    $pdf->Cell(15);
    $pdf->SetFont('Times','',9);
    $pdf->Cell(50, 8, 'Note: *) Pengajuan Budget 1-5jt');
    $pdf->Cell(40, 8, '**) Pengajuan Budget > 5jt');
    $pdf->Ln();
    $pdf->Ln();
		if ($i < 5) {
            $pdf->Cell(15);
