...
if(isset($_POST['simpan'])){
    try{
        ...
        if($id_goods <> ""){
            if($id_goods > 0){
                $sql = "UPDATE tb_goods SET name=:goods, price_estimate=:price_estimate, quantity_unit=:quantity_unit, active=:aktif, last_modified=:last_modified, modified_by=:modified_by,
                   id_role=:group, id_unit=:unit where id_goods=:id_goods";
                $query = $dbcon->prepare($sql);
                $query->bindParam("goods", $goods, PDO::PARAM_STR);
            ...
            $sql = "INSERT INTO tb_goods VALUES (null, :goods, 0, :quantity, :price_estimate, :price_estimate, :aktif, :created, null, :created_by, 0, :group, :unit)";
            $query = $dbcon->prepare($sql);
            $query->bindParam("goods", $goods, PDO::PARAM_STR);
            $query->bindParam("quantity_unit", $quantity_unit, PDO::PARAM_STR);
            $query->bindParam("price_estimate", $price_estimate, PDO::PARAM_STR);
